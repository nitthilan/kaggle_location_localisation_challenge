...
def makeprobtable(train, feature, threshold):
    table = train.groupby('place_id')[feature].value_counts().reset_index()
    count = train.groupby('place_id')[feature].count()

    print table
    print count
    for table_idx in table:
        print table_idx
        print table[table_idx], count[table[table_idx]]
        table[table_idx] = table[table_idx]/count[table_idx]
...
train = pd.read_csv('../../data/train.csv',dtype={'place_id': np.int64}, index_col = 0) 
...
if cross_validation==1: 
    ...
    max3placeids[['row_id','place_id']].to_csv('../../data/submission.csv', header=True, index=False )
